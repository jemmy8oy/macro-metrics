...
public class MetricsService : IMetricsService
{
    ...
    public RatioResponse GetRatio(string numerator, string denominator)
    {
        ...
        return new RatioResponse(numerator, denominator, Math.Round(series.Average(p => p.Value), 2), series);
    }

    public IndicatorResponse? GetIndicator(string id)
    {
        ...
        return new IndicatorResponse(id, meta.Label, meta.Unit, Math.Round(series.Average(p => p.Value), 2), series);
    }
